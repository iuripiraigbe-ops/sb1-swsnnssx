...
// Health check with real database and Redis checks
fastify.get('/health', async () => {
  try {
    ...
    return {
      status: 'ok',
      db: Array.isArray(dbOk) && dbOk.length > 0 ? 'ok' : 'fail',
      redis: redisOk === 'PONG' ? 'ok' : 'fail',
      timestamp: new Date().toISOString(),
    };
  } catch (error) {
    fastify.log.error('Health check failed: %s', error instanceof Error ? error.message : String(error));
    return {
      status: 'error',
